...
def update_leaderboard(file_name, leader_names, leader_scores, player_name, player_score):
    ...
    if (len(player_name) > 5):
        str(player_name).pop(5)
        str(player_score).pop(5)
...
def draw_leaderboard(high_scorer, leader_names, leader_scores, turtle_object, player_score):
    ...
    elif (player_score >= silver_score and player_score < silver_score):
      turtle_object.write("You earned a silver medal!", font=font_setup)
    elif (player_score >= bronze_score and player_score < bronze_score):
